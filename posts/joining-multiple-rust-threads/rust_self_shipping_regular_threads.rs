...
// `spawn_self_shipping_thread` even if the self-shipping thread panics.
struct SendOnDrop<R> {
    handle: Option<JoinHandle<R>>,
    tx: mpsc::Sender<JoinHandle<R>>,
}

impl<R> Drop for SendOnDrop<R> {
    fn drop(&mut self) {
...
pub fn spawn_self_shipping_thread<F, R>(tx: mpsc::Sender<JoinHandle<R>>, func: F)
where
    ...
    R: Send + 'static,
{
    ...
    let handle = std::thread::spawn(move || {
        ...
        SendOnDrop {
            handle: Some(handle),
            // It is responsibility of the caller to make sure that the `rx` side of this channel
            // is alive until after this thread is finished.
            tx,
        };
...
fn main() {
    // Create the channel that the new threads will use to send their handles to the main thread.
    let (handle_tx, handle_rx) = mpsc::channel();

    // Spawn the new threads.
    spawn_self_shipping_thread(handle_tx.clone(), || target_fn(1));
    spawn_self_shipping_thread(handle_tx.clone(), || target_fn(3));
    spawn_self_shipping_thread(handle_tx.clone(), || target_fn(5));
...
mod tests {
    ...
    mod spawn_self_shipping_thread {
        use super::*;
